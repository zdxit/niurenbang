...
var validatePrompt = {
    username:{
        ...
        isNull:"请输入用户名",
        error:{
            beUsed:"该用户名已被使用，请使用其它用户名注册，如果您是&quot;{1}&quot;，请<a href='/index.php/Index/tiao/name/login' class='blue'>登录</a>",
        ...
        if (!length && format) {
            ...
            if (!namestate || nameold != option.value) {
                if (nameold != option.value) {
                    ...
                    ///AjaxService.php?action=CheckUnicknme&uid=
                   // $.getJSON("/index.php/Index/AjaxService/action/name/uid/" + escape(option.value), function(date) {
                    $.post("/index.php/Index/AjaxService",{action:'name',uid:escape(option.value)}, function(date) {
                    	if (date == 0) {
            ...
            if (!format2) {
                ...
                if (!emailstate || emailold != option.value) {
                    if (emailold != option.value) {
                        ...
                        option.errorEle.html("<span style='color:#999'>检验中……</span>");
                        // $.getJSON("/index.php/Index/AjaxService/action/email/uid/" + escape(option.value), function(date) {
                        $.post("/index.php/Index/AjaxService",{action:'email',uid:escape(option.value)}, function(date) {
                        	if (date == 0) {//alert(option.value);
                                validateSettings.succeed.run(option);
                                emailstate = true;
                            } else {
        ...
        if (!authcodestate || authcodeold != option.value) {
            if (authcodeold != option.value) {
               ...
               //$.getJSON("/AjaxService.php?action=CheckAuthcode&str=" + escape(option.value), function(date) {
                $.post("/index.php/Index/AjaxService",{action:'yzm',uid:escape(option.value)}, function(date) {
                	//alert(date);
                    if (date == 0) {
                        validateSettings.succeed.run(option);
                        authcodestate = true;
